...
def load_data(path):
    ...
    predicate2id, id2predicate = dict(), dict()
    with open(path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    for d in data:
        for s in d["text"]:
            if s not in char2id:
                char2id[s] = len(char2id) + 2
                id2char[len(id2char) + 2] = s
        spo_list = []
        for dd in d["spo_list"]:
            spo_list.append(tuple(dd))
            if dd[1] not in predicate2id:
                predicate2id[dd[1]] = len(predicate2id)
                id2predicate[len(id2predicate)] = dd[1]
        D.append({'text': d["text"], 'spo_list': spo_list})
    return D, char2id, id2char, predicate2id, id2predicate


# 加载数据集
train_data, _, _, predicate2id, id2predicate = load_data(os.path.join(project_path, 'data/train_triples.json'))
valid_data, _, _, _, _ = load_data(os.path.join(project_path, 'data/dev_triples.json'))
